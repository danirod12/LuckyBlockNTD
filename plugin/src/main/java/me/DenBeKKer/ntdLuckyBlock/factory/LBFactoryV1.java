...
public class LBFactoryV1 implements LBFactory {
    ...
    public void generate(FileConfiguration file, LuckyBlockType type) {
        generate(file, type, 5, 50);
    }
    ...
    public String generateItem() {
        ...
        mat = Arrays.asList(Material.values()).stream().filter(n -> {
        ...
        }).filter(n -> !n.name().toUpperCase().contains("AIR")).collect(Collectors.toList());
