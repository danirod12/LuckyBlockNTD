...
public enum Hook {
    ...
    public static void print() {
        ...
        for (Hook hook : Hook.values()) {
            if (hook.error == null)
                LBMain.log(Level.INFO, ChatColor.GREEN + hook.name() + " connected");
            else disabled.add(ChatColor.RED + hook.name() + " " + hook.error);
        }

        for (String message : disabled) {
            LBMain.log(Level.INFO, message);
        }

        LBMain.log(Level.INFO, ChatColor.GOLD + "Found " + (Hook.values().length - disabled.size())
